...
void Moon::Render(float theta)
{
	...
	ImGui::SliderFloat("al", &al, 0.0f, 10.0f);
	sAlpha->SetFloat(al);
...
float Moon::GetAlpha(float theta)
{
	...
	{
		//std::cout << sinf(theta) << " " << fabsf(sinf(theta + Math::PI / 2.0f)) << std::endl;
...
D3DXMATRIX Moon::GetTransform(float theta)
{
	...
	D3DXMatrixTranslation(&T, playerPosition.z, playerPosition.y, playerPosition.z);
